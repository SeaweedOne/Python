...
def readBlockchain(blockchainFilePath, mode = 'internal'): #디폴트값 없기때문에 파라미터 들어와야함 / 모드는 디폴트값이 있어 호출시 없어도 ㄱ
    print("readBlockchain")
    importedBlockchain = []

    try:
        with open(blockchainFilePath, 'r',  newline='') as file: #파일을 열어서 new line이 개행문자 구분해줘서 라인바이라인으로 읽음 읽어서 file에 넣자.
            blockReader = csv.reader(file) #블록리더 = 리스트
            for line in blockReader: #포문을 돌며 읽어온 컬럼 하나하나를 블록으로 생성함.
                block = Block(line[0], line[1], line[2], line[3], line[4], line[5]) #블록은 객체
        ...
        if mode == 'internal' : #인터널 - 내부적으로 필요에 의해서 호출된거고 그 외에는 엘스문으로 들어감 보통은 위의 트라이 문에서 끝난다. (이라인은 보통 제네시스 블록을 호출할 때만 사용)
            blockchain = generateGenesisBlock() # 빈 배열에 제네시스 블록이라는 객체를 넣는다
            importedBlockchain.append(blockchain)
            writeBlockchain(importedBlockchain) #이곳에서 파일로 던짐
            return importedBlockchain
        else :
            return None
...
def mineNewBlock(difficulty=g_difficulty, blockchainPath=g_bcFileName): #파라미터 주지 않아도 여기서 설정한 디폴트값이 들어감
    blockchain = readBlockchain(blockchainPath) # 패스 - 파일명 (리드블록체인 펑션을 호출함)
    strTxData = getTxData()
    if strTxData == '' :
                ...
                try:
                    ...
                    if block == None:
                        ...
                        try:
                            ...
                            # 해결 4. 사용자에게 안내문구 출력 후 startPoint를 마지막 인덱스로 변경
                            if startPoint > len(block):
                                print("인덱스 오류. 설정할 수 있는 startPoint의 최대 범위는 {} 입니다.".format(len(block) -1 ))
                                data.append("인덱스 오류. 설정할 수 있는 startPoint의 최대 범위는 {} 입니다.".format(len(block) -1 ))
                                startPoint = len(block) -1

                            # 예외 5 : endPoint 값이 블록의 길이보다 길거나 0보다 작을경우.
                            # 해결 5-1 : 사용자에게 블록의 길이 알려줌.
                            # 해결 5 -2 : endPoint를 블록의 길이로 바꿔준다.
                            if  endPoint > len(block) or endPoint < 0 :
                                print("인덱스 에러. {} 개의 블록이 존재합니다.".format(len(block)))
                                data.append("인덱스 에러. {} 개의 블록이 존재합니다.".format(len(block)))
                                endPoint = len(block)
                            ...
                            if (endPoint - startPoint) > 100 : # 리스트 분할 출력을 위한 for문
                                ...
                                for i in range(start, end, augmenter):  # for문을 통해 리스트를 잘라서 출력
                                    ...
                                    if printList != []:  # printList가 빈 리스트가 아니라면 if문 수행
                                        for j in range(len(printList)):  # printList의 길이만큼 for문 수행
                                            print(printList[j])  # printList의 요소 하나씩 출력
                                        start = index  # 시작값을 마지막 인덱스 값으로 바꿔줌
                                        userInput = input("계속 출력을 원하시면 아무키나 입력하세요.") # 사용자에게 계속 출력할지 선택하게함. 선택의 범위를 넗히기위해 특정 문자가 아닌 아무 문자나 입력할 수 있도록 함.
                                        if len(userInput) > 0: # 사용사의 입력 문자의 길이가 0 이상이라면 (한글자라도 입력했다면 ) 계속 수행
                                            continue
                                        else: # 아무것도 입력하지 않았으면 안내문구 출력 후 종료
                                            print("출력을 종료합니다.")
                                            break
                            else : #출력해할 범위가 100보다 작은경우
                                for i in range(startPoint, endPoint): #최종 startPoint와 endPoint를 이용해 for문.
